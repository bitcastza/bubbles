...
class BCDTests(TestCase):
    def test_next_service(self):
        ...
        period = datetime.timedelta(weeks=54)
        bcd = BCD.objects.create(number='1',
                                 ...
                                 size=BCD.SMALL)
        self.assertEqual(bcd.next_service, last_service + period)
...
class CylinderTests(TestCase):
    def test_next_service(self):
        ...
        cylinder = Cylinder.objects.create(number='1',
                                 ...
                                 last_viz=last_viz,
                                 last_hydro=last_viz,
                                 viz_period=datetime.timedelta(weeks=54),
                                 hydro_period=datetime.timedelta(weeks=108))

        self.assertEqual(cylinder.next_service, last_viz + period)
...
class BCDTests(TestCase):
    def test_next_service(self):
        ...
        regulator = Regulator.objects.create(number='1',
                                 ...
                                 last_service=last_service)
        self.assertEqual(regulator.next_service, last_service + period)
