...
class StaticFunctionTest(TestCase):
    def test_get_initial_period_exists(self):
        ...
        rental_period = baker.make(
            RentalPeriod, start_date=start_date, end_date=end_date, hidden=False
        )
...
class EquipmentTableWidgetTest(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.rental = Rental(
            rental_period=rental_period, user=user, state=Rental.REQUESTED, deposit=0
        )
    ...
    def test_get_context_hidden(self):
        hidden_item = baker.make(Item, state=Item.AVAILABLE, hidden=True)
        context = self.table.get_context("Equipment table", None, None)
        context = context["widget"]
        item_types = [x["description"] for x in context["item_types"]]
    ...
    def test_get_context_free(self):
        free_item = baker.make(Item, state=Item.AVAILABLE, free=True)
        context = self.table.get_context("Equipment table", None, None)
        context = context["widget"]
...
class RentalEquipmentListFieldTest(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.rental = Rental(
            rental_period=rental_period, user=user, state=Rental.RENTED, deposit=0
        )
        cls.rental.save()
        cls.rental_item = RentalItem(item=cls.item, rental=cls.rental, cost=0)
        cls.rental_item.save()

    def test_return_rented_item_after_second_rental(self):
        start_date = datetime.date(year=2018, month=3, day=1)
        returned_rental_period = RentalPeriod(
            start_date=start_date,
        ...
        returned_rental = Rental(
            rental_period=returned_rental_period,
            user=user,
            state=Rental.RETURNED,
            deposit=0,
        )
        returned_rental.save()
        returned_rental_item = RentalItem(
            item=self.item, rental=returned_rental, cost=0, returned=True
        )
    ...
    def test_multple_rental_item_error(self):
        dup_item = RentalItem(item=self.item, rental=self.rental, cost=0)
        dup_item.save()
        form = RentalEquipmentListField()
...
class EquipmentFormTest(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.item.save()
        cls.request_item = RequestItem(item_description=cls.item.description, cost=25)
        start_date = datetime.date.today()
    ...
    def test_request_hood(self):
        ...
        string = "period={period}&belt_weight=0&{key}={description}&{key}=N/A&{key}=N/A&{key}={cost}&liability=on".format(
            period=self.rental_period.pk,
            key="equipment-0",
            description=self.request_item.item_description,
            cost=self.request_item.cost,
        )

        data = QueryDict(string)
        form = forms.RequestEquipmentForm(user=user, data=data)
