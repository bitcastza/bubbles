...
class RentalAdminTest(TestCase):
    ...
    def setUpTestData(cls):
        ...
        user = User.objects.create_user(username="jacob")
        cls.rental_period = RentalPeriod.objects.create(
    ...
    def test_deposit(self):
        self.assertEqual(
            self.admin.deposit(self.rental_period),
            f"R{self.rental_period.default_deposit}",
        )

    def test_cost_per_item(self):
        self.assertEqual(
            self.admin.cost_per_item(self.rental_period),
            f"R{self.rental_period.default_cost_per_item}",
        )
...
class RentalItemAdminTest(TestCase):
    ...
    def test_number(self):
        self.assertEqual(self.admin.number(self.rental_item), self.item.number)

    def test_description(self):
        self.assertEqual(
            self.admin.description(self.rental_item), self.item.description
        )
    ...
    def test_user(self):
        self.assertEqual(self.admin.user(self.rental_item), self.user)
...
class RequestItemAdminTest(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.request_item = RequestItem(
            rental=rental, item_description="Test description", item_size="M", cost=25
        )
        cls.admin = RequestItemAdmin(RequestItem, admin.sites.site)

    def test_number(self):
        self.assertEqual(
            self.admin.size(self.request_item), self.request_item.item_size
        )

    def test_description(self):
        self.assertEqual(
            self.admin.description(self.request_item),
    ...
    def test_user(self):
        self.assertEqual(self.admin.user(self.request_item), self.user)
